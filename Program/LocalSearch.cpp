...
void LocalSearch::run(Country * cntry, double penaltyCapacityLS, double penaltyDurationLS)
{
	...
	// Register the solution produced by the LS in the country
	exportCountry(cntry);
}
...
void LocalSearch::loadCountry(Country * cntry)
{
	emptyRoutes.clear();
	nbMoves = 0; 
	for (int r = 0; r < params->nbVehicles; r++)
	{
		...
		if (!cntry->chromR[r].empty())
		{
			Node * myClient = &clients[cntry->chromR[r][0]];
			myClient->route = myRoute;
			myClient->prev = myDepot;
			myDepot->next = myClient;
			for (int i = 1; i < (int)cntry->chromR[r].size(); i++)
			{
				Node * myClientPred = myClient;
				myClient = &clients[cntry->chromR[r][i]]; 
...
void LocalSearch::exportCountry(Country * cntry)
{
	...
	for (int r = 0; r < params->nbVehicles; r++)
	{
		cntry->chromR[r].clear();
		Node * node = depots[routePolarAngles[r].second].next;
		while (!node->isDepot)
		{
			cntry->chromT[pos] = node->cour;
			cntry->chromR[r].push_back(node->cour);
			node = node->next;
			pos++;
		}
	}
