...
void Split::generalSplit(Country * cntry, int nbMaxVehicles)
{
	...
	{
		cliSplit[i].demand = params->cli[cntry->chromT[i - 1]].demand;
		cliSplit[i].serviceTime = params->cli[cntry->chromT[i - 1]].serviceDuration;
		cliSplit[i].d0_x = params->timeCost[0][cntry->chromT[i - 1]];
		cliSplit[i].dx_0 = params->timeCost[cntry->chromT[i - 1]][0];
		if (i < params->nbClients) cliSplit[i].dnext = params->timeCost[cntry->chromT[i - 1]][cntry->chromT[i]];
	...
	if (splitSimple(cntry) == 0)
		splitLF(cntry);

	// Build up the rest of the Country structure
	cntry->evaluateCompleteCost();
}

int Split::splitSimple(Country * cntry)
{
	...
	for (int k = maxVehicles - 1; k >= 0; k--)
	{
		...
		for (int ii = begin; ii < end; ii++)
			cntry->chromR[k].push_back(cntry->chromT[ii]);
		end = begin;
	}
...
int Split::splitLF(Country * cntry)
{
